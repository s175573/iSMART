...
## Create Numpy array to store the Di amino acid interaction matrix
DDcor={}
ff=open('./DDcor.txt')
AllLines=ff.readlines()
DDaa=AllLines[0].strip().split('\t')
AAseq='-'.join(DDaa)
DDaa_dict={}
count=0
for mm in DDaa:
...
for kk in xrange(1,401):
    xx=np.array(AllLines[kk].strip().split('\t')[1:]).astype(np.float)
    for jj in xrange(0,400):
        m1=DDaa[kk-1]
        m2=DDaa[jj]
        DDcor[(m1,m2)]=float(xx[jj])
#    DDcor[kk-1]=xx
#    DDcor1d+=list(xx)

## Pre-calculated correlation threshold for di-amino acid replacement
COR_000=1
COR_0005=0.6284302
COR_001=0.5905098
COR_002=0.5543462
...
def ObtainCL(InputFile,VScore, gap, gapn, cutoff=7, UseV=True, outDir='./',Nthread=1,Di=False):
    ...
    for line in ALLLines[1:]: ## get rid of header line
        ...
        ID.append(count)
        count+=1
    TMP=PWalign(Seqs,Vgene,ID,VScore, gap, gapn, UseV=UseV,cutoff=cutoff,Nthread=Nthread,Di=Di)
    PWscore=TMP[0]
    Seqs=TMP[1]
    Vgene=TMP[2]
    ID=TMP[3]
